...
const navigate = useNavigate();
    React.useEffect(() => {
        axios.post('http://localhost:8000/getforms',{role:'NSS'})
        .then(res => {
            setForms(res.data)
        })
    },[])
    ...
    return (
        <div className="bg-slate-100">
          ...
          <div className="container mx-auto px-6 py-12">
            <h1 className="text-3xl md:text-5xl font-semibold mb-12 text-center">National Service Scheme</h1>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              <div
                onClick={() => {
                  navigate('/dashboard', { state: { role: 'NSS', fname: 'National Service Scheme' } });
                }}
